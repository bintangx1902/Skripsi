...
with tf.device('/GPU:0'):
    train_df['data'] = train_df['filepath'].apply(
        lambda x: preprocess_image(x, (110, 128), preprocess_function=tf.keras.applications.vgg19.preprocess_input, augment=True))
    img_test_df['data'] = img_test_df['filepath'].apply(
        lambda x: preprocess_image(x, (110, 128), preprocess_function=tf.keras.applications.vgg19.preprocess_input)
    )
...
# ## Creating Model
#%%
base_model = tf.keras.applications.vgg19.VGG19(weights='imagenet', include_top=False, input_shape=(256, 110, 3), pooling='max')
#%% md
# ## Input Layer
#%%
...
fc1 = tf.keras.layers.Dense(1024, activation='relu')(x)
