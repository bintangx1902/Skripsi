...
_, x_temp, _, y_temp = train_test_split(
    x_audio_train, y_audio_train, test_size=0.5, random_state=100, stratify=y_audio_train
)
del _
...
x_img_train, x_img_val, y_img_train, y_img_val = train_test_split(x_img, y_img, test_size=0.2, random_state=100,
...
def create_base_model(inputs: tf.keras.layers.Input, instance_name: str) -> tf.keras.Model:
    ...
    outputs = base(inputs)
    return tf.keras.Model(inputs, outputs, name=f"vgg19_{instance_name}")
#%%
