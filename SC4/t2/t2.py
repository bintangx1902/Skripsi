...
x_img_train, x_img_val, y_img_train, y_img_val = train_test_split(x_img, y_img, test_size=0.3, random_state=19,
                                                                  shuffle=True)
del x_img, y_img
#%% md
# # Modeling
...
history = model.fit(
    ...
    verbose=2,
    batch_size=128,
    steps_per_epoch=len(x_img_train) // 128,
    validation_steps=len(x_img_val) // 128,
)
